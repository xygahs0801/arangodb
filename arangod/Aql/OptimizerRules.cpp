...
void arangodb::aql::distributeInClusterRule(Optimizer* opt,
  ...
  LOG_TOPIC(ERR,Logger::FIXME) << "enter distribute in cluster rule";
  ...
  if (arangodb::ServerState::instance()->isCoordinator()) {
    ...
    for (ExecutionNode* root : nodes) {
      ...
      if (collInfo->isSmart() && collInfo->type() == TRI_COL_TYPE_EDGE) {
        ...
        LOG_TOPIC(ERR,Logger::FIXME) << "exit distribute in cluster smart";
      ...
      if (originalParent != nullptr) {
        ...
        if (!isSubQuery){
          ...
          LOG_TOPIC(ERR,Logger::FIXME) << "update subquery root with parents";
  ...
  opt->addPlan(std::move(plan), rule, wasModified);
  LOG_TOPIC(ERR,Logger::FIXME) << "exit distribute in cluster rule";
}
