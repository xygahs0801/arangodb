...
void SingleServerEdgeCursor::getDocAndRunCallback(OperationCursor* cursor ,Callback callback) {
  ...
  if (collection->readDocument(_trx, etkn->token(), *_mmdr)) {
    ...
    _opts->cache()->increaseCounter(); // was macht das?
    // run callback (advances cursor?!)
...
void SingleServerEdgeCursor::advanceCursor(OperationCursor*& cursor, std::vector<OperationCursor*>& cursorSet, bool& noMoreItems){
  noMoreItems = false;
...
bool SingleServerEdgeCursor::next(std::function<void(std::unique_ptr<EdgeDocumentToken>&&, VPackSlice, size_t)> callback) {
  ...
  // // skip empty cursor sets
  // while (cursorSet.empty()) {
  //   // TODO - use advanceCursor
  ...
  do {
    if (cursorSet.empty() || !cursor->hasMore()) {
      ...
      advanceCursor(cursor, cursorSet, noMoreItems);
      if (noMoreItems) {
        return false;
      }
    } else {
  ...
  LOG_TOPIC(ERR, Logger::FIXME) << "this should not happen";
  return false;
}
